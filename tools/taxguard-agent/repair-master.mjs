...
process.stdin.once("data", async (answer) => {
   ...
   * validate the ENTIRE proposal against the current source before
   * writing even one TaxGuard source file.
   */
  console.log(`
============================================================
 MASTER REPAIR V2 - PATCH PREFLIGHT
============================================================
`);
  ...
  try {
    ...
    for (let index = 0; index < proposal.edits.length; index++) {
      ...
      if (
        ...
        throw new Error(
          `Edit ${index + 1} is malformed.`
        );
      }

      safePath(edit.path);

      if (!simulatedFiles.has(edit.path)) {
        throw new Error(
          `Edit ${index + 1}: target file does not exist: ${edit.path}`
        );
      }

      if (edit.oldText.length === 0) {
        throw new Error(
          `Edit ${index + 1}: oldText is empty.`
        );
      }

      if (edit.oldText === edit.newText) {
        throw new Error(
          `Edit ${index + 1}: replacement makes no change.`
        );
      }

      const current = simulatedFiles.get(edit.path);

      const first = current.indexOf(edit.oldText);

      if (first === -1) {
        throw new Error(
          `Edit ${index + 1}: oldText was not found in ${edit.path}.`
        );
      }
      ...
      if (second !== -1) {
        throw new Error(
          `Edit ${index + 1}: oldText is not unique in ${edit.path}.`
        );
      }
      ...
      simulatedFiles.set(edit.path, simulated);
    }

    console.log(
      `PREFLIGHT PASS: ${proposal.edits.length} edits validated.`
    );

  } catch (preflightError) {
    console.error("");
    console.error("============================================================");
    console.error(" MASTER REPAIR V2 - PREFLIGHT REJECTED");
    console.error("============================================================");
    console.error("");
    console.error(
      preflightError?.message || preflightError
    );
    console.error("");
    console.error("NO TAXGUARD SOURCE FILE WAS MODIFIED.");
    console.error("Generate a fresh proposal before trying again.");
    process.exit(1);
  }
