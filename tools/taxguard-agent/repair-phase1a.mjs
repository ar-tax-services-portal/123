...
Modify ONLY src/context/AppContext.tsx.

Repair ONLY the client authentication/session boundary.

Required changes:

1. Remove Michael Perotti as a LIVE Firebase fallback.
2. Remove m.perotti@example.com as a LIVE Firebase fallback.
3. Remove 678-205-9486 as a LIVE Firebase fallback.
4. Remove Perotti Capital Holdings LLC as a LIVE Firebase fallback.

5. Do NOT construct a LIVE TaxGuard application user directly from
   Firebase Auth fields.

6. Firebase onAuthStateChanged may observe Firebase authentication,
   but the TaxGuard LIVE application identity must come from the
   server firebase-session endpoint.

7. When a Firebase user exists and there is no valid TaxGuard
   application session, obtain the Firebase ID token and restore the
   TaxGuard session through api.auth.firebaseSession.

8. Do not allow the Firebase listener to overwrite an already
   provisioned TaxGuard LIVE currentUser.

9. When Firebase reports no authenticated user, clear LIVE identity
   safely when appropriate.

10. Preserve the existing artest2026 demonstration login.

11. LIVE registration must NOT fall back to api.auth.register when
    Firebase registration fails.

12. Preserve the existing successful Firebase registration ->
    firebaseSession provisioning path.

13. Preserve permanent Client ID returned by the TaxGuard server.

14. Preserve Stage 01, Stage 02, Stage 03 and Intelligence Core.

15. Preserve external-submission hard stop.

16. Make only minimal targeted edits.

17. Do NOT modify document upload, appointments, onboarding,
    messaging, Stage 01, Stage 02, Stage 03, dashboard components,
    Firestore schemas or Client ID allocation in this run.

18. Do NOT perform broad formatting or refactoring.
...
if (proposal.edits.length > 12) {
  fail(
    `Proposal contains ${proposal.edits.length} edits. Phase 1A limit is 12.`
  );
}
